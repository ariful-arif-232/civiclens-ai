...
export async function uploadImage(id, bytes, config, fetcher = fetch) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const reason = message.includes('api key') ? 'api-key'
        : message.includes('signature') ? 'signature'
        : message.includes('auth') || response.status === 401 ? 'auth'
        : response.status === 403 ? 'forbidden'
        : 'provider-rejected';
      console.error('[cloudinary] upload rejected', { status: response.status, reason });
      throw new Error('Upload failed.');
    }
    if (result.public_id !== publicId) {
      console.error('[cloudinary] upload response mismatch', { reason: 'public-id' });
      throw new Error('Invalid upload response.');
    }
    if (result.resource_type !== 'image') {
      console.error('[cloudinary] upload response mismatch', { reason: 'resource-type' });
      throw new Error('Invalid upload response.');
    }
    if (!isReportImageUrl(result.secure_url, config.cloudName, publicId)) {
      console.error('[cloudinary] upload response mismatch', { reason: 'secure-url' });
      throw new Error('Invalid upload response.');
    }
